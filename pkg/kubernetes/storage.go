...
func NewStorageCommand(nano v1.Nanokube) *StorageCommand {
	...
	command := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			switch backend {
			case "etcd3":
				return c.RunEtcd3(cmd.Context())
			default:
				return fmt.Errorf("unsupported storage backend: %q", backend)
			}
		},
	}
	command.Flags().String("backend", "etcd3", "Storage backend to use. Supported: etcd3.")
	c.Command = newCommand(nano, command)
	return c
}

func (c *StorageCommand) RunEtcd3(ctx context.Context) error {
