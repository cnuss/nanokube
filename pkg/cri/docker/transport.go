...
func (t *loggingTransport) RoundTrip(req *http.Request) (*http.Response, error) {
	...
	if err != nil {
		evt := logger.Trace().Str("method", method).Str("url", url).Err(err)
		if reqBuf.Len() > 0 {
	...
	if resp.Body != nil && zerolog.GlobalLevel() <= zerolog.TraceLevel {
		...
		evt := logger.Trace().Str("method", method).Str("url", url).Int("status", resp.StatusCode)
		if reqBuf.Len() > 0 {
			evt.Str("reqBody", reqBuf.String())
		}
		evt.Msg("dockerapi")
...
func (w *wiretapReader) Close() error {
	err := w.reader.Close()
	evt := logger.Trace().Str("method", w.method).Str("url", w.url).Int("status", w.status)
	if w.reqBody != "" {
