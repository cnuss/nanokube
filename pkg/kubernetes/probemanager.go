...
func (ProbeManager) AddPod(_ context.Context, pod *v1.Pod) {
	probeLog.Debug().Str("pod", pod.Name).Msg("AddPod not implemented")
}

func (ProbeManager) StopLivenessAndStartup(pod *v1.Pod) {
	probeLog.Debug().Str("pod", pod.Name).Msg("StopLivenessAndStartup not implemented")
}

func (ProbeManager) RemovePod(pod *v1.Pod) {
	probeLog.Debug().Str("pod", pod.Name).Msg("RemovePod not implemented")
}

func (ProbeManager) CleanupPods(_ map[types.UID]sets.Empty) {
	probeLog.Debug().Msg("CleanupPods not implemented")
}

func (ProbeManager) UpdatePodStatus(_ context.Context, _ *v1.Pod, podStatus *v1.PodStatus) {
	probeLog.Debug().Msg("UpdatePodStatus not implemented")
